...
#[cfg(not(windows))]
fn main() -> ExitCode {
    eprintln!("sample-tool is a Windows/PE-side test binary");
    ExitCode::from(64)
}
...
mod win {
    ...
    pub(super) fn main() -> ExitCode {
        ...
        if cooperative && let Err(code) = install_via_loadlibrary() {
            return code;
        }
    ...
    fn run_dynamic() -> ExitCode {
        ...
        unsafe {
            let open: OpenProcFn =
                std::mem::transmute(resolve(b"kernel32.dll\0", b"OpenProcess\0"));
            let rpm: RpmFn =
                std::mem::transmute(resolve(b"kernel32.dll\0", b"ReadProcessMemory\0"));
            let wpm: WpmFn =
                std::mem::transmute(resolve(b"kernel32.dll\0", b"WriteProcessMemory\0"));
