...
the Wine internals it is **forbidden** to touch (and why), and is honest about the
one residual fragile spot and the one known coverage hole.
...
The injection/interposition vector is chosen by the Phase 3 spike (ADR-0006). Most
...
The exception: **if the spike lands on inline-hooking the `Nt*` prologues** —
...
the forbidden list above), but the specific prologue bytes/length can shift between
Wine builds, so an inline hook may need re-validation per Wine version. This is the
single known fragile point, and it is called out here precisely so that, if ADR-0006
...
## 4. The known hole: raw syscalls bypass export-level interception
...
This is a real, accepted limitation, not a bug to paper over:

- Decant does **not** claim to cover raw-syscall tools. The docs and diagnostics
  state this plainly (Phase 3 red-team in `docs/TESTING.md` exercises it).
- Closing it would require operating at the syscall-dispatch layer, which is
  exactly the Wine-internal territory forbidden in §2 — so doing so would trade
  version-portability for coverage. Decant deliberately keeps the portability.
- Such a call still cannot escape the **execution wall** (`docs/ARCHITECTURE.md`
  §3): even a raw syscall in the *guest* cannot make memflow run guest code. The
  hole is about *interception visibility* in the Wine-hosted tool, not about giving
  the tool new powers over the guest.
