...
pub(crate) unsafe fn redirect(name: *const u8) -> *mut c_void { unsafe {
    if iat::cstr_eq(name, b"GetProcessImageFileNameA") { return get_process_image_file_name_a as *mut c_void; }
    if iat::cstr_eq(name, b"K32GetProcessImageFileNameA") { return get_process_image_file_name_a as *mut c_void; }
    if iat::cstr_eq(name, b"GetProcessImageFileNameW") { return get_process_image_file_name_w as *mut c_void; }
    if iat::cstr_eq(name, b"K32GetProcessImageFileNameW") { return get_process_image_file_name_w as *mut c_void; }
    if iat::cstr_eq(name, b"QueryFullProcessImageNameA") { return query_full_process_image_name_a as *mut c_void; }
    if iat::cstr_eq(name, b"QueryFullProcessImageNameW") { return query_full_process_image_name_w as *mut c_void; }
    if iat::cstr_eq(name, b"IsWow64Process") { return is_wow64_process as *mut c_void; }
    if iat::cstr_eq(name, b"GetProcessId") { return get_process_id as *mut c_void; }
    if iat::cstr_eq(name, b"GetExitCodeProcess") { return get_exit_code_process as *mut c_void; }
    core::ptr::null_mut()
}}
...
pub unsafe fn install() -> u32 { unsafe {
    ...
    let img_a = {
        ...
        if p != 0 { p } else { resolve(&k32, b"K32GetProcessImageFileNameA\0") }
    };
    store(&GET_PROCESS_IMAGE_FILE_NAME_A_ORIG, img_a);
    let img_w = {
        let p = resolve(&psapi, b"GetProcessImageFileNameW\0");
        if p != 0 { p } else { resolve(&k32, b"K32GetProcessImageFileNameW\0") }
    };
    store(&GET_PROCESS_IMAGE_FILE_NAME_W_ORIG, img_w);
    store(&QUERY_FULL_PROCESS_IMAGE_NAME_A_ORIG, resolve(&k32, b"QueryFullProcessImageNameA\0"));
    store(&QUERY_FULL_PROCESS_IMAGE_NAME_W_ORIG, resolve(&k32, b"QueryFullProcessImageNameW\0"));
    store(&IS_WOW64_PROCESS_ORIG, resolve(&k32, b"IsWow64Process\0"));
    store(&GET_PROCESS_ID_ORIG, resolve(&k32, b"GetProcessId\0"));
    store(&GET_EXIT_CODE_PROCESS_ORIG, resolve(&k32, b"GetExitCodeProcess\0"));

    let mut total = 0u32;
    total += iat::patch_all_modules(None, b"GetProcessImageFileNameA", get_process_image_file_name_a as *mut c_void);
    total += iat::patch_all_modules(None, b"K32GetProcessImageFileNameA", get_process_image_file_name_a as *mut c_void);
    total += iat::patch_all_modules(None, b"GetProcessImageFileNameW", get_process_image_file_name_w as *mut c_void);
    total += iat::patch_all_modules(None, b"K32GetProcessImageFileNameW", get_process_image_file_name_w as *mut c_void);
    total += iat::patch_all_modules(None, b"QueryFullProcessImageNameA", query_full_process_image_name_a as *mut c_void);
    total += iat::patch_all_modules(None, b"QueryFullProcessImageNameW", query_full_process_image_name_w as *mut c_void);
