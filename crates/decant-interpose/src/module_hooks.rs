...
use crate::hooks::{MemoryBasicInformation, put_ansi_counted, put_wide_counted};
use crate::iat;
use crate::rpc;
...
type GetModInfoFn =
    unsafe extern "system" fn(*mut c_void, *mut c_void, *mut ModuleInformation, u32) -> i32;
type EnumModsExFn =
    unsafe extern "system" fn(*mut c_void, *mut *mut c_void, u32, *mut u32, u32) -> i32;
...
type NtQvmFn = unsafe extern "system" fn(
    *mut c_void,
    *const c_void,
    u32,
    *mut c_void,
...
pub(crate) unsafe fn redirect(name: *const u8) -> *mut c_void {
    unsafe {
        if iat::cstr_eq(name, b"GetModuleInformation") {
            return get_module_information as *mut c_void;
        }
        if iat::cstr_eq(name, b"K32GetModuleInformation") {
            return get_module_information as *mut c_void;
        }
        if iat::cstr_eq(name, b"EnumProcessModulesEx") {
            return enum_process_modules_ex as *mut c_void;
        }
        if iat::cstr_eq(name, b"K32EnumProcessModulesEx") {
            return enum_process_modules_ex as *mut c_void;
        }
        if iat::cstr_eq(name, b"GetMappedFileNameA") {
            return get_mapped_file_name_a as *mut c_void;
        }
        if iat::cstr_eq(name, b"K32GetMappedFileNameA") {
            return get_mapped_file_name_a as *mut c_void;
        }
        if iat::cstr_eq(name, b"GetMappedFileNameW") {
            return get_mapped_file_name_w as *mut c_void;
        }
        if iat::cstr_eq(name, b"K32GetMappedFileNameW") {
            return get_mapped_file_name_w as *mut c_void;
        }
        if iat::cstr_eq(name, b"NtQueryVirtualMemory") {
            return nt_query_virtual_memory as *mut c_void;
        }
        core::ptr::null_mut()
    }
}
...
pub unsafe fn install() -> u32 {
    unsafe {
        ...
        GET_MODULE_INFORMATION_ORIG.store(
            first(
                resolve(&psapi, b"GetModuleInformation\0"),
                resolve(&k32, b"K32GetModuleInformation\0"),
            ),
            Ordering::SeqCst,
        );
        ENUM_PROCESS_MODULES_EX_ORIG.store(
            first(
                resolve(&psapi, b"EnumProcessModulesEx\0"),
                resolve(&k32, b"K32EnumProcessModulesEx\0"),
            ),
            Ordering::SeqCst,
        );
        GET_MAPPED_FILE_NAME_A_ORIG.store(
            first(
                resolve(&psapi, b"GetMappedFileNameA\0"),
                resolve(&k32, b"K32GetMappedFileNameA\0"),
            ),
            Ordering::SeqCst,
        );
        GET_MAPPED_FILE_NAME_W_ORIG.store(
            first(
                resolve(&psapi, b"GetMappedFileNameW\0"),
                resolve(&k32, b"K32GetMappedFileNameW\0"),
            ),
            Ordering::SeqCst,
        );
        NT_QUERY_VIRTUAL_MEMORY_ORIG
            .store(resolve(&ntdll, b"NtQueryVirtualMemory\0"), Ordering::SeqCst);
