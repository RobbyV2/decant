...
pub(crate) unsafe fn cstr_eq(ptr: *const u8, want: &[u8]) -> bool {
    unsafe {
        let mut i = 0usize;
        loop {
            let c = *ptr.add(i);
...
unsafe fn is_system_module(base: *mut c_void) -> bool {
    unsafe {
        ...
        let export_dir = opt_hdr.add(
            OPT_HDR_DATA_DIR_OFFSET
                + DIRECTORY_ENTRY_EXPORT * core::mem::size_of::<ImageDataDirectory>(),
...
pub unsafe fn patch_module_iat(
    ...
    unsafe {
        ...
        let import_dir = opt_hdr.add(
            OPT_HDR_DATA_DIR_OFFSET
                + DIRECTORY_ENTRY_IMPORT * core::mem::size_of::<ImageDataDirectory>(),
        ) as *const ImageDataDirectory;
        ...
        if !psapi.is_null() {
            ...
            if !enum_proc.is_null() {
                ...
                if enum_proc(GetCurrentProcess(), mods.as_mut_ptr(), cb, &mut needed) != 0 {
                    let count =
                        (needed as usize / core::mem::size_of::<*mut c_void>()).min(mods.len());
                    for &m in &mods[..count] {
